...
import kotlinx.android.synthetic.main.item_add_new_examination.view.*
...
class AddNewExaminationVH(private val view: View, private val listener: IItemListener) :
  ...
  fun bindData(position: Int, isSelect: Boolean) = with((view)) {
    checkSelectedNewExamination(isSelect)
    llContainerItemExaminationNew.setOnClickListener {
      listener.onItemChoice(position)
    }
  }

  private fun checkSelectedNewExamination(selected: Boolean) = with(view) {
    if (selected) {
      imgSelectItemNewExamination.show()
      return@with
    }
    imgSelectItemNewExamination.hide()
  }
}
